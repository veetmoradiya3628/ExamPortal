...
export class OrgAdminProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.orgadminId = this._userService.getLoggedInUserId();
  ...
  loadUserDetails() {
    this._adminApiService.getUserDetails(this.orgadminId).subscribe(
      (res: any) => {
  ...
  updateUserPassword() {
    if (this.updateUserPasswordForm.valid) {
      ...
      this._generalService.resetUserPassword(this.orgadminId, requestObj).subscribe(
        (res: any) => {
          console.log(res);
